...
class GAN(nn.Module):
    ...
    def calculate_D_loss(self, z, real_data):
        ...
        fake_label = torch.zeros(len(fake_data_score), device=self.device)
        real_label = torch.ones(len(real_data_score), device=self.device)
        D_loss = self.loss(fake_data_score, fake_label) + self.loss(real_data_score, real_label)
        return D_loss
    ...
    def calculate_G_loss(self, z):
        ...
        fake_data = self.G(z)
        fake_data_score = self.D(fake_data)
        real_label = torch.ones(len(fake_data_score), device=self.device)
        G_loss = self.loss(fake_data_score, real_label)
        return G_loss
