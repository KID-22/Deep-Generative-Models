...
def GAN_Train(opt):
    ...
    model = GAN(data_dim=opt.data_dim, hidden_dim=opt.hidden_dim, z_dim=opt.z_dim, device=opt.device)
    model.to(opt.device)
    ...
    for epoch in range(1, opt.max_epoch+1):
        ...
        if epoch % opt.verbose == 0:
            ...
            save_image(dataset_name=opt.dataset, model_name="GAN", epoch=epoch, real_data=valid_data, sample_data=sample_data)
    ...
    # save model
    model.save_model("../ckpt/" + opt.dataset + "/GAN/best_model")

    # save loss
    np.savetxt("../result/" + opt.dataset + "/GAN/full_train_G_loss.txt", np.array(full_train_G_loss))
    np.savetxt("../result/" + opt.dataset + "/GAN/full_train_D_loss.txt", np.array(full_train_D_loss))
    np.savetxt("../result/" + opt.dataset + "/GAN/full_valid_G_loss.txt", np.array(full_valid_G_loss))
    np.savetxt("../result/" + opt.dataset + "/GAN/full_valid_D_loss.txt", np.array(full_valid_D_loss))

    print("\n=============best model=============")
    model.load_model("../ckpt/" + opt.dataset + "/GAN/best_model")
    model.eval()
...
if __name__ == "__main__":
    ...
    os.makedirs("../result/" + opt.dataset + "/GAN/images/", exist_ok=True)
    os.makedirs("../ckpt/" + opt.dataset + "/GAN/", exist_ok=True)
